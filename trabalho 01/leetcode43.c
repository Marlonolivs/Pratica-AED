...
/*
====================
Rude <gmp.h>                // GMP, BigInt e similares
...
char *multiply( char *num1, char *num2 ) {
	...
	// Se algum dos números começar com 0,
    // o resultado será 0
	if(num1[0] == '0' || num2[0] == '0'){
		char *zero = malloc(2*sizeof(char));
		if	(!zero) return NULL; 
		zero[0] = '0'; // Coloca o caractere '0'
		zero[1] = '\0'; // Finaliza a string

		return zero;	

	}

	int n1 = strlen(num1);// Tamanho da primeira string
	int n2 = strlen(num2);	// Tamanho da segunda string
	int len = n1 + n2;  // Tamanho máximo possível do resultado

	char *res = malloc((len+1)*sizeof(char));
	memset(res,'0',len); // Inicializa o resultado com zeros
	res[len] = '\0';

	for(int i = n1-1; i >= 0 ; i--){ // Percorre num1 do final para o início
		for(int j = n2-1; j >= 0 ; j--){// Percorre num2 do final para o início

			// Converte os caracteres para números
            // e multiplica os dois dígitos
			int mul = (num1[i] - '0') * (num2[j] - '0');

			int pos1 = i+j; // Posição do valor que é carregado para a próxima casa
			int pos2 = i + j + 1; // Posição do resultado atual

			// Soma a multiplicação com o valor
            // que já existe naquela posição
			int sum = mul +(res[pos2] - '0');

			res[pos2] = (sum % 10) + '0';// Guarda o último dígito
			res[pos1] += (sum / 10);// Guarda o valor que é carregado para a próxima casa
		}
	}

	int start = 0;

	// Ignora os zeros do início
	while(start < len && res[start] == '0'){
		start ++;
	}

	// Se existem zeros no início,
    // remove eles movendo a string
	if (start > 0) {
